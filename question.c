void displayQuestion(SDL_Renderer *renderer, TTF_Font *font, char *filename, char *correctAnswer) {
    ...
    for (int i = 0; i < 10; i++) {
        running = 1;
        isCorrect = 0;
        memset(responseField.text, 0, sizeof(responseField.text)); // Réinitialiser le champ de réponse
        cursorStartTime = SDL_GetTicks(); // Réinitialiser le timer du curseur
        cursorVisible = 1;
            ...
            while (SDL_PollEvent(&event)) {
                if (event.type == SDL_QUIT) {
                    ...
                    if (responseField.is_active && event.key.keysym.sym == SDLK_BACKSPACE) {
                        ...
                        running = 0; // Passer à la question suivante
                    }
                }
            }
